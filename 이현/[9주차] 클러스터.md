...
Redis Cluster는 데이터를 노드에 직접 매핑하지 않고 해시슬롯(Hash Slot)에 매핑한다.
...
을 기준으로 동작하기 때문에 노드가 추가되더라도 전체 데이터를 재분배할 필요가 없다.
...
이 가능하기 때문에 운영 중에도 리샤딩(Resharding)을 수행할 수 있다.
